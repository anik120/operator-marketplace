...
func NewConfiguringReconciler(log *logrus.Entry, reader datastore.Reader, client client.Client, cache Cache) Reconciler {
	return NewConfiguringReconcilerWithInterfaceClient(log, reader, wrapper.NewClient(client), cache)
}
...
func NewConfiguringReconcilerWithInterfaceClient(log *logrus.Entry, reader datastore.Reader, client wrapper.Client, cache Cache) Reconciler {
...
func (r *configuringReconciler) Reconcile(ctx context.Context, in *marketplace.CatalogSourceConfig) (out *marketplace.CatalogSourceConfig, nextPhase *marketplace.Phase, err error) {
	...
	csReconciler := catalogsource.NewResourcesReconciler(r.log, r.reader, r.client)

	key := client.ObjectKey{
		Name:      in.Name,
		Namespace: in.Namespace,
	}
	err = csReconciler.ReconcileCatalogSourceResources(key, in.Spec.DisplayName, in.Spec.Publisher, in.Spec.TargetNamespace, in.Spec.Packages, in.Labels)
	if err != nil {
