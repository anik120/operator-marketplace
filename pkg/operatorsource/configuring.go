...
func (r *configuringReconciler) Reconcile(ctx context.Context, in *marketplace.OperatorSource) (out *marketplace.OperatorSource, nextPhase *marketplace.Phase, err error) {
	...
	csReconciler := catalogsource.NewResourcesReconciler(r.logger, r.reader, r.client)

	key := client.ObjectKey{
		Name:      in.Name,
		Namespace: in.Namespace,
	}
	err = csReconciler.ReconcileCatalogSourceResources(key, in.Spec.DisplayName, in.Spec.Publisher, in.Namespace, packages, in.Labels)
	if err != nil {
		nextPhase = phase.GetNextWithMessage(phase.Configuring, err.Error())
		return
	}
